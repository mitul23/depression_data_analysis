...
MDD_sig <- fread("results/PGC_UKB_depression_genome-wide.fix.LDclumped.tsv", sep="\t",header=T)%>% 
...
harmonised <- read.csv("results/mr_base_harmonised_data.csv") %>% as_tibble()
...
IVW <- mr_results[c(3,8),]## %>% mutate(b_modified = exp(c(0.2752064,0.3438404))) # modified second value to be positive to check what the 95%-CI is.
...
exp(cbind("Odds ratio"= coef(IVW$b)))
forest(m_,studlab = T)
m_ <- metagen(exp.b, 
                 se, 
                 studlab = id.outcome,
